...
class RuntimeSettings(BaseModel):
    ...
    remake: ProductSettings
    morphology: Union[GALFITSettings, ImcascadeSettings, PysersicSettings]
    ficls: list[FICL]
    roots: PathSettings
    date_time: datetime
    run_number: int = 1
    ...
    def setup_directories(self,initialized:bool=True):
        ...
        # If inputs already initialized, only create output and product
        # directories
        if initialized:
            for ficl in self.ficls:
                for required_input_directory_name in 
...
class MorphitRuntimeasdfsd(BaseModel):
    """Runtime settings for a single program execution of MorphFITS.
...
def set_run_settings(config_dict: dict) -> dict:
    ...
    config_dict["datetime"] = datetime_type.now()
